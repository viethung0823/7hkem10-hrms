...
export default () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [companiesData, departmentsData, locationsData] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await apiService.createJobPosition(formData);
      history.push('/job-positions');
    } catch (error) {
      console.error('Error creating job position:', error);
      setError(error.message || 'Failed to create job position');
    }
  };
      ...
      <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div className="d-block mb-4 mb-md-0">
          <Breadcrumb className="d-none d-md-inline-block" listProps={{ className: "breadcrumb-dark breadcrumb-transparent" }}>
            ...
            <Breadcrumb.Item active>New Job Position</Breadcrumb.Item>
          </Breadcrumb>
          <h4>New Job Position</h4>
        </div>
      </div>
      ...
      <Card border="light" className="bg-white shadow-sm">
        <Card.Body>
          <Form onSubmit={handleSubmit}>
            ...
            <div className="mt-3">
              <Button variant="primary" type="submit">Create Job Position</Button>
            </div>
          </Form>
        </Card.Body>
      </Card>
