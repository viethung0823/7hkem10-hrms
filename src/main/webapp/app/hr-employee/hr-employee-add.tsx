...
export default function HrEmployeeAdd() {
  ...
  useDocumentTitle(t('hrEmployee.add.headline'));
  ...
  const createHrEmployee = async (data: HrEmployeeDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/hrEmployees', data);
      navigate('/hrEmployees', {
            state: {
              msgSuccess: t('hrEmployee.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="d-flex flex-wrap mb-4">
      <h1 className="flex-grow-1">{t('hrEmployee.add.headline')}</h1>
      <div>
        <Link to="/hrEmployees" className="btn btn-secondary">{t('hrEmployee.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createHrEmployee)} noValidate>
      ...
      <input type="submit" value={t('hrEmployee.add.headline')} className="btn btn-primary mt-4" />
    </form>
  </>);
}
