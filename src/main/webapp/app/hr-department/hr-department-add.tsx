...
export default function HrDepartmentAdd() {
  const { t } = useTranslation();
  useDocumentTitle(t('hrDepartment.add.headline'));
  ...
  const createHrDepartment = async (data: HrDepartmentDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/hrDepartments', data);
      navigate('/hrDepartments', {
            state: {
              msgSuccess: t('hrDepartment.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="d-flex flex-wrap mb-4">
      <h1 className="flex-grow-1">{t('hrDepartment.add.headline')}</h1>
      <div>
        <Link to="/hrDepartments" className="btn btn-secondary">{t('hrDepartment.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createHrDepartment)} noValidate>
      ...
      <input type="submit" value={t('hrDepartment.add.headline')} className="btn btn-primary mt-4" />
    </form>
  </>);
}
